class User < ActiveRecord::Base
  ...
  scope :with_rings,         -> { select('users.*, (SELECT created_at FROM rings WHERE user_id = users.id ORDER BY created_at desc) AS last_ring_created_at').joins('LEFT OUTER JOIN rings ON rings.user_id = users.id ').group('users.id').having('count(rings.id) > 0') }
